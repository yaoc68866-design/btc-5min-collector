...
## v2.3.0 — 采样率提升 3 倍 + 命令行参数支持 (2026-06-12)

### 新增
- ✅ **argparse 命令行参数**：`--interval`（采样间隔）、`--depth`（订单簿深度）、`--batch`（批量写入）、`--data-dir`（数据目录）
- ✅ **采样率提升 3 倍**：默认间隔从 1 秒改为 **0.33 秒**（3 次/秒）
- ✅ 订单簿深度和批量大小现在可通过 CLI 配置（之前硬编码为 10 档 / 50 条）

### 部署
- 服务器实测 ~5-6.7 快照/秒（含新加坡→美国 API 延迟）
